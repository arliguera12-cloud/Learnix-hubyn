-- =============================================================
-- LEARNIX DTE HUB — SaaS Multi-tenant Schema  v2.0
-- =============================================================
...
-- ─────────────────────────────────────────────────────────────
-- [1] TABLA: organizaciones  (el "tenant" central del SaaS)
-- ─────────────────────────────────────────────────────────────
...
-- Miembros de la org pueden ver su propia organización
CREATE POLICY "org_select_miembros" ON organizaciones
    FOR SELECT USING (
        id IN (SELECT organizacion_id FROM perfiles WHERE id = auth.uid())
    );

-- Solo admins pueden actualizar datos de su organización
CREATE POLICY "org_update_admin" ON organizaciones
    FOR UPDATE USING (
        id IN (
            SELECT organizacion_id FROM perfiles
...
-- ─────────────────────────────────────────────────────────────
-- [2] TABLA: perfiles  (un registro por usuario auth.users)
-- ─────────────────────────────────────────────────────────────
...
CREATE POLICY "perfil_select_admin_org" ON perfiles
    FOR SELECT USING (
        organizacion_id = (
            SELECT p2.organizacion_id FROM perfiles p2
...
-- ─────────────────────────────────────────────────────────────
-- [HELPERS] Funciones SECURITY DEFINER para RLS sin recursión
-- ─────────────────────────────────────────────────────────────
...
-- Retorna true si el usuario autenticado es admin de su org
...
-- Retorna el plan de suscripción de la organización activa
...
-- ─────────────────────────────────────────────────────────────
-- [3] TABLA: clientes  (empresas auditadas — nivel organización)
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS clientes (
    ...
    created_at       TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(organizacion_id, nit)    -- NIT único por organización
);
...
-- ─────────────────────────────────────────────────────────────
-- [4] TABLA: dte_procesados  (historial de DTEs — nivel org)
-- ─────────────────────────────────────────────────────────────
...
-- Cualquier miembro de la org puede ver los DTEs
...
-- Solo admins pueden borrar DTEs
...
-- ─────────────────────────────────────────────────────────────
-- [5] TRIGGER principal: Auto-crear perfil + org en registro
-- ─────────────────────────────────────────────────────────────
--
-- Metadata que debe enviar el cliente al hacer sign_up:
--
--   Flujo Admin (crea nueva firma):
--     raw_user_meta_data = {
...
--       "rol":             "contador"    -- opcional, default 'contador'
--     }
-- ─────────────────────────────────────────────────────────────
...
DECLARE
    ...
    v_rol         TEXT;
BEGIN
    -- Extraer metadata enviada durante sign_up
    v_nombre    := COALESCE(
    ...
    ELSE
        ...
        EXCEPTION
            WHEN invalid_text_representation THEN
                ...
                INSERT INTO organizaciones (nombre) VALUES ('Org de ' || split_part(NEW.email, '@', 1))
                RETURNING id INTO v_org_id;
                v_rol := 'admin';
        END;
...
-- ─────────────────────────────────────────────────────────────
-- [6] TRIGGER: updated_at automático en organizaciones
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- [7] TRIGGER: Contador mensual de DTEs procesados por org
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- [8] FUNCIÓN HELPER: Verificar límite de DTEs antes de insertar
-- (Llámala desde Python antes de guardar_dte_db si quieres
--  bloquear cuando se supera el límite del plan)
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- [9] CRON: Reset mensual del contador de DTEs
-- Actívalo en Supabase Dashboard → Database → Cron Jobs → New cron job
-- Expression: 0 0 1 * *   (primer día de cada mes, medianoche UTC)
-- ─────────────────────────────────────────────────────────────
...
-- =============================================================
-- SECCIÓN DE MIGRACIÓN DESDE v1.0
-- Solo ejecutar si ya tienes las tablas anteriores con datos.
-- Descomenta bloque a bloque y ejecuta en orden.
-- =============================================================
/*

-- PASO M1: Agregar columnas a tablas existentes (idempotente)
...
-- PASO M2: Eliminar políticas de la v1.0
...
-- PASO M3: Crear una org por cada usuario existente y migrar sus datos
DO $$
DECLARE
    r        RECORD;
    v_org_id UUID;
...
BEGIN
    ...
    LOOP
        v_nombre := 'Firma de ' || split_part(r.email, '@', 1);

        INSERT INTO organizaciones (nombre, plan_suscripcion, email_contacto)
        VALUES (v_nombre, 'starter', r.email)
        RETURNING id INTO v_org_id;
        ...
        -- Sincronizar contador de DTEs del mes actual
        UPDATE organizaciones
...
-- PASO M4: Eliminar constraint antigua de clientes (user_id, nit)
--          y crear la nueva (organizacion_id, nit)
...
-- PASO M5: Hacer NOT NULL las columnas migradas
