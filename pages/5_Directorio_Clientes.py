...
# --- HEADER ---
st.markdown("<h2 style='font-family: Courier New, monospace; color: #FF4B4B; letter-spacing: 2px;'>YN</h2>", unsafe_allow_html=True)
...
with col1:
    ...
    with st.form("form_nuevo_cliente", clear_on_submit=True):
        f_nit = st.text_input("NIT o DUI*", placeholder="06141234567890 o 12345678-1")
        f_nombre = st.text_input("Razón Social*", placeholder="Mi Empresa S.A. de C.V.")
        f_dui = st.text_input("DUI (Opcional)", placeholder="12345678-1")
        f_nrc = st.text_input("NRC (Opcional)", placeholder="123456")
        f_actividad = st.text_input("Giro/Actividad (Opcional)", placeholder="Comercio minorista")
...
with col2:
    ...
    if db_clientes:
        ...
        with st.expander("🗑️ Zona de Peligro — Eliminar Empresa", expanded=False):
            st.warning("⚠️ Esta acción no se puede deshacer. Las facturas se mantendrán pero la empresa desaparecerá del menú.")
            
            opciones_borrar = ["-- Selecciona para eliminar --"] + list(db_clientes.keys())
            formato = lambda x: f"{x} → {db_clientes[x]['nombre']}" if x != "-- Selecciona para eliminar --" else x
...
with col_stats2:
    cliente_activo_count = 1 if isinstance(cliente_activo, dict) and cliente_activo.get("nit") in db_clientes else 0
