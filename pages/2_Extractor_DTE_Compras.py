...
# ─────────────────────────────────────────────
# 7. EXTRACCIÓN DE NOMBRE DEL PROVEEDOR (FILTRADO)
# ─────────────────────────────────────────────
def extraer_nombre_proveedor(texto_completo: str, nit_prov: str, pos_nit: int, cliente_activo: dict) -> str:
    ...
    def limpiar(s: str) -> str:
        try:
            ...
            s = re.sub(
                r"^[\s\-:]*(?:RAZ[OÓ]N\s*SOCIAL|NOMBRE(?:\s+[o/y]?\s+RAZ[OÓ]N\s+SOCIAL)?|"
    ...
    def valido(s: str) -> bool:
        try:
            ...
            if len(T) < 4 or len(T) > 90: return False
            if nombre_receptor and (T == nombre_receptor or T.startswith(nombre_receptor[:15])): return False
            if any(b in T for b in BASURA_ESTRICTA): return False
            if es_linea_direccion(T): return False
            
            # EL NUEVO ESCUDO PARA BLOQUEAR METADATOS DEL DTE
            malas_frases = [
                "FECHA", "HORA", "MODELO", "TIPO", "SELLO", "CÓDIGO", "CODIGO", "DOC.", "DOCUMENTO",
                "NÚMERO", "NUMERO", "INFORMACIÓN", "INFORMACION", "PÁGINA", "PAGINA", "CONDICION",
                "REGISTRO", "TELEFONO", "TELÉFONO", "CORREO", "EMAIL", "DIRECCION", "DIRECCIÓN", "FAX"
            ]
            if any(T.startswith(p) for p in malas_frases): return False
            
            for b in PALABRAS_BASURA:
                if b in T and len(b) > 5: return False
            if T in NOMBRES_INVALIDOS: return False
            digitos = sum(c.isdigit() for c in T)
            if len(T) > 0 and (digitos / len(T)) > 0.35: return False
    ...
    try:
        # Estrategia A: Búsqueda de la etiqueta en el bloque EMISOR
        m_emisor_bloque = re.search(r"(?i)\bEMISOR\b(.*?)(?:\bRECEPTOR\b|\bCLIENTE\b)", texto_completo, re.S)
        bloque_busqueda = m_emisor_bloque.group(1) if m_emisor_bloque else texto_completo[:1500]
        
        m_nom_etq = re.search(r"(?i)(?:Nombre(?:\s+[o/y]?\s+raz[oó]n\s+social)?|Raz[oó]n\s+social)\s*[:\n]+\s*([^\n]+)", bloque_busqueda)
        if m_nom_etq:
            candidato = limpiar(m_nom_etq.group(1))
            if not valido(candidato):
                m_nom_multi = re.search(r"(?i)(?:Nombre(?:\s+[o/y]?\s+raz[oó]n\s+social)?|Raz[oó]n\s+social)\s*[:\n]+\s*(?:[^\n]+\n)?\s*([^\n]+)", bloque_busqueda)
                if m_nom_multi:
                    candidato = limpiar(m_nom_multi.group(1))
            if valido(candidato): return candidato

        # Estrategia B: Buscar sufijos corporativos
        m_corp = re.search(r"(?i)^([^\n]{5,80}(?:S\.A\.\s*DE\s*C\.V\.|S\.A\.|LTDA\.?|SOCIEDAD\s+ANONIMA|S\.A\.S\.)[^\n]*)", texto_completo[:1500], re.M)
        if m_corp:
            candidato = limpiar(m_corp.group(1))
            if valido(candidato): return candidato

        # Estrategia C: Geometría - Línea superior al NIT
        lineas_todas = [ln.strip() for ln in texto_completo.split('\n') if ln.strip()]
        for i, linea in enumerate(lineas_todas):
            if nit_prov and nit_prov in re.sub(r'[^0-9]', '', linea):
                if i > 0:
        ...
        # Estrategia D: Palabras Comerciales Sueltas
        for linea in lineas_todas[:30]:
