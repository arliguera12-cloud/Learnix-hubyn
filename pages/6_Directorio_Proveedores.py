...
# --- HEADER ---
st.markdown("<h2 style='font-family: Courier New, monospace; color: #003057; letter-spacing: 2px;'>YN</h2>", unsafe_allow_html=True)
st.title("🏢 Directorio de Proveedores")
st.write("**Vendor Master Data** — Base maestra de proveedores que usarán automáticamente los extractores.")
st.divider()

# --- TABS ---
tab1, tab2, tab3 = st.tabs(["➕ Agregar / Actualizar", "📋 Base de Datos", "🚀 Carga Masiva (Excel)"])
...
with tab2:
    ...
    if db_proveedores:
        ...
        formato_elim = lambda x: f"{x} → {db_proveedores[x]['nombre']}" if x != "-- Selecciona --" else x
...
with tab3:
    ...
    if archivo_subido:
        try:
            ...
            with col2:
                col_nrcs = st.selectbox("Columna NRC (Opcional)", ["-- Ninguna --"] + list(df_import.columns), key="col_nrcs")
            with col3:
            ...
            if st.button("🚀 Inyectar al Sistema", type="primary", use_container_width=True):
                ...
                for index, row in df_import.iterrows():
                    try:
                        ...
                        nrc_raw = str(row[col_nrcs]) if col_nrcs != "-- Ninguna --" and pd.notna(row[col_nrcs]) else ""
                ...
                # Resumen
                st.success(f"✅ **Inyección completada!**")
