...
export default function Compras() {
    ...
    try {
      ...
      if (isMultiple) {
        ...
        const { resultados: res, errores } = await subirLoteEnTandas(
          filesOrFile, procesarComprasLote, dId, nombre, avanzarProgreso, nrc, dui,
        )
        nuevos = res
      ...
      guardarResultados('compras', dId, agregados).then(ids => {
        if (!ids) return
  ...
  async function handleCorregir(row, cambios, soloConforme) {
    ...
    if (row.dbId) await actualizarResultado('compras', row.dbId, registroNuevo)
  }
    ...
    <div className="max-w-[100rem] mx-auto space-y-5">
      ...
      <div>
        ...
        <h2 className="text-3xl text-fg leading-none flex items-center gap-3">
          <IconCompras className="w-7 h-7 text-accent" />
          Compras
        </h2>
            ...
            {tab === 4 && (
              <div className="space-y-2">
                {alertas.length === 0 ? (
                  ...
                  alertas.map((r, i) => (
                    <ResultadosTabla
                      key={i} data={r} tipo="compras" declaranteId={declaranteId}
