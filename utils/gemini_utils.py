"""
Learnix Hub — Groq utility v1.0 (reemplaza Gemini).
Modelo: llama3-8b-8192 vía Groq Cloud.

Mantiene la misma interfaz pública que gemini_utils.py para compatibilidad
con todas las páginas importadoras.
"""
...
def gemini_disponible() -> bool:
    """Retorna True si la API key de Groq está configurada."""
...
PASO 2 ▶ DISTINGUE ETIQUETAS DE VALORES REALES
  ...
  Registra en razonamiento.etiqueta_vs_valor qué identificaste.

PASO 3 ▶ LIMPIA EL VALOR BRUTO
  Elimina: prefijos residuales, UUIDs, horas, marcas de versión, guiones al inicio/final.
...
PASO 4 ▶ AUTO-VALIDACIÓN
  ...
  Registra resultado en razonamiento.autovalidacion.
...
# ─── Llamada central a Groq ────────────────────────────────────────────────────

def _llamar_groq(prompt: str) -> dict | None:
    """
    Envía el prompt a Groq usando llama3-8b-8192.
    Fuerza respuesta JSON con response_format={"type": "json_object"}.
    Retorna el dict parseado o None en caso de error.
    """
    ...
    for attempt in range(_MAX_RETRIES):
        try:
            ...
            raw = raw.strip()
            # Defensa ante markdown residual
            ...
            if "rate_limit" in msg.lower() or "429" in msg:
                _ultimo_error = "Límite de tasa de Groq alcanzado. Espera un momento."
